...
public class UISpeedometer : MonoBehaviour {
	...
	void LateUpdate () {
		...
		velocityText.text = Mathf.Round( boatVelocity / 2.9f ).ToString();

		float newRot = boatVelocity / 15f;
		Quaternion newSpeedDialPivotRot = Quaternion.Euler( 0f, 0f, -(newRot*111f) );			// -135 because the dial rotates between 0 and -135
